...
print('spa_en_mBERT: model training beginning...')
...
spa_model = AutoModelForTokenClassification.from_pretrained(
    'liaad/srl-en_mbert-base', num_labels=spa_num_labels, id2label=spa_id2label, label2id=spa_label2id
)

# Set Spanish model parameters & create Trainer()
spa_training_args = TrainingArguments(
    output_dir='spa_en_mBERT/',
    learning_rate=2e-5,
...
print('spa_en_mBERT: model training complete')

spa_model.save_pretrained('spa_en_mBERT/best_performing')
tokenizer.save_pretrained('spa_en_mBERT/best_performing')

print('gal_ensp_mBERT: model training beginning...')
        
# import Galician data
gal_ds = load_dataset('mbruton/galician_srl')
gal_label_list = gal_ds['train'].features['tags'].feature.names

tokenizer = AutoTokenizer.from_pretrained('spa_en_mBERT/best_performing', local_files_only=True)
...
gal_model = AutoModelForTokenClassification.from_pretrained(
    'spa_en_mBERT/best_performing', local_files_only=True, num_labels=gal_num_labels, id2label=gal_id2label, label2id=gal_label2id, ignore_mismatched_sizes=True
)

# Set Galician model parameters & create Trainer()
gal_training_args = TrainingArguments(
    output_dir='gal_ensp_mBERT/',
    learning_rate=2e-5,
    per_device_train_batch_size=16,
    per_device_eval_batch_size=16,
    num_train_epochs=100,
...
print('gal_ensp_mBERT: model training complete')