...
def spa_compute_metrics(p):
    predictions, labels = p
    predictions = np.argmax(predictions, axis=2)

    true_predictions = [
        [spa_label_list[p] for (p, l) in zip(prediction, label) if l != -100]
        for prediction, label in zip(predictions, labels)
    ]
    true_labels = [
        [spa_label_list[l] for (p, l) in zip(prediction, label) if l != -100]
        for prediction, label in zip(predictions, labels)
    ]
...
print('spa_enpt_mBERT: model training beginning...')

# import Spanish data
spa_ds = load_dataset('mbruton/spanish_srl')
spa_label_list = spa_ds['train'].features['tags'].feature.names

tokenizer = AutoTokenizer.from_pretrained('liaad/srl-enpt_mbert-base')

# Tokenize Spanish Data
tokenized_spa_ds = spa_ds.map(tokenize_and_align_labels, batched=True)

data_collator = DataCollatorForTokenClassification(tokenizer=tokenizer)
seqeval = evaluate.load('seqeval')

# Create Spanish id2label/label2id
spa_id2label, spa_label2id = set_id_label_dicts(spa_label_list)
spa_num_labels = len(spa_id2label)

# Load Spanish base model
spa_model = AutoModelForTokenClassification.from_pretrained(
    'liaad/srl-enpt_mbert-base', num_labels=spa_num_labels, id2label=spa_id2label, label2id=spa_label2id
)

# Set Spanish model parameters & create Trainer()
spa_training_args = TrainingArguments(
    output_dir='spa_enpt_mBERT/',
    learning_rate=2e-5,
...
spa_trainer = Trainer(
    model=spa_model,
    args=spa_training_args,
    train_dataset=tokenized_spa_ds['train'],
    eval_dataset=tokenized_spa_ds['dev'],
    tokenizer=tokenizer,
    data_collator=data_collator,
    compute_metrics=spa_compute_metrics,
...
# Test Spanish model
final_evaluate(spa_model, spa_trainer, tokenized_spa_ds['test'])

print('spa_enpt_mBERT: model training complete')

spa_model.save_pretrained('spa_enpt_mBERT/best_performing')
tokenizer.save_pretrained('spa_enpt_mBERT/best_performing')

print('gal_enptsp_mBERT: model training beginning...')
        
# import Galician data
gal_ds = load_dataset('mbruton/galician_srl')
gal_label_list = gal_ds['train'].features['tags'].feature.names

tokenizer = AutoTokenizer.from_pretrained('spa_enpt_mBERT/best_performing', local_files_only=True)
...
gal_model = AutoModelForTokenClassification.from_pretrained(
    'spa_enpt_mBERT/best_performing', local_files_only=True, num_labels=gal_num_labels, id2label=gal_id2label, label2id=gal_label2id, ignore_mismatched_sizes=True
)

# Set Galician model parameters & create Trainer()
gal_training_args = TrainingArguments(
    output_dir='gal_enptsp_mBERT/',
    learning_rate=2e-5,
    per_device_train_batch_size=16,
    per_device_eval_batch_size=16,
    num_train_epochs=100,
...
print('gal_enptsp_mBERT: model training complete')